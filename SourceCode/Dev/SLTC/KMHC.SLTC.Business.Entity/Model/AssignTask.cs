...
namespace KMHC.SLTC.Business.Entity.Model
{
    ...
    public class AutoTaskTmp
    {
        public static Dictionary<string, TaskInfoTmp> AutoTask = new Dictionary<string, TaskInfoTmp>{
          ...
          {AutoTaskType.SPMSQ.ToString(),new TaskInfoTmp(){ModuleName="SPMSQ评估",Content="簡易心智量表评估(此提醒由系统生成)"}},
          {AutoTaskType.IADL.ToString(),new TaskInfoTmp(){ModuleName="IADL评估",Content="IADL评估(此提醒由系统生成)"}},
          {AutoTaskType.SORE.ToString(),new TaskInfoTmp(){ModuleName="SORE评估",Content="壓瘡風險评估(此提醒由系统生成)"}},
          {AutoTaskType.FALL.ToString(),new TaskInfoTmp(){ModuleName="FALL评估",Content="跌倒危險因數评估(此提醒由系统生成)"}},
          {AutoTaskType.KARNOFSKY.ToString(),new TaskInfoTmp(){ModuleName="KARNOFSKY评估",Content="柯氏量表评估(此提醒由系统生成)"}},
          {AutoTaskType.BEHAVIOR.ToString(),new TaskInfoTmp(){ModuleName="BEHAVIOR评估",Content="行為認知评估(此提醒由系统生成)"}},
          {AutoTaskType.NursingDemand.ToString(),new TaskInfoTmp(){ModuleName="NursingDemand评估",Content="護理需求评估(此提醒由系统生成)"}},
           {AutoTaskType.GDS.ToString(),new TaskInfoTmp(){ModuleName="憂鬱量表评估",Content="憂鬱量表评估(此提醒由系统生成)"}}
      };
