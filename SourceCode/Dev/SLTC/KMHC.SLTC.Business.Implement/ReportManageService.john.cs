...
namespace KMHC.SLTC.Business.Implement
{
    public partial class ReportManageService : BaseService, IReportManageService
    {
        ...
        public void DownloadInfectionStatisticsFile(int year, string templateFile)
        {
            ...
            SheetParameterContainer sheetParameterContainer = new SheetParameterContainer() { SheetName = "統計總表" };
            dynamic[] categorys = new dynamic[] { 
                new { category = "001", rowIndex = 7 },      //上呼吸道感染人次 
                new { category = "002", rowIndex = 9 },      //下呼吸道感染人次 
                new { category = "003", rowIndex = 14 },    //使用導尿管感染人次 
                new { category = "004", rowIndex = 17 },    //未使用导尿管感染人次
                new { category = "005", rowIndex = 19 },    //疥瘡感染人次 
            ...
            elementFormatters.Add(new CellFormatter(sheetParameterContainer["year"], ADToMinYear(year,true)));
            ...
            #endregion
            ExportHelper.ExportToWeb(templateFile, Path.GetFileName(templateFile), new SheetFormatter("統計總表", elementFormatters.Where(it => it != null).ToArray()));
        }
        ...
        private void SetQuestionData(Question question)
        {
            if (question != null)
            {
                ...
                {
                    question.CDTW = string.Format("{0}/{1}/{2}", question.CreateDate.Value.Year - 1911, question.CreateDate.Value.Month, question.CreateDate.Value.Day);
                }
                if (question.NextDate.HasValue)
                {
                    question.NDTW = string.Format("{0}/{1}/{2}", question.NextDate.Value.Year - 1911, question.NextDate.Value.Month, question.NextDate.Value.Day);
                }
