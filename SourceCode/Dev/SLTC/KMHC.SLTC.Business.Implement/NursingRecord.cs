...
namespace KMHC.SLTC.Business.Implement
{
    public class NursingRecord : BaseService, INursingRecord
    {
        ...
        public BaseResponse insertReferralLis(TranSferVisit baseRequest)
        {
            ...
            // baseRequest.CREATEBY = SecurityHelper.CurrentPrincipal.Account;
        ...
        public BaseResponse<IList<CommFile>> QueryCommFile(BaseRequest<RecordFilter> request)
        {
            ...
            List<LTC_COMMFILE> regQuestion = new List<LTC_COMMFILE>();
            if (String.IsNullOrEmpty(request.Data.Name))
            {
                regQuestion = unitOfWork.GetRepository<LTC_COMMFILE>().dbSet.ToList();
            }
            else
            {
                regQuestion = unitOfWork.GetRepository<LTC_COMMFILE>().dbSet.Where(m => m.TYPENAME.Contains(request.Data.Name)).ToList();
        ...
        public BaseResponse<IList<CommFile>> GetCommfile(string id)
        {
            ...
            List<LTC_COMMFILE> regQuestion = unitOfWork.GetRepository<LTC_COMMFILE>().dbSet.Where(m => m.TYPENAME == id).ToList();
            Mapper.CreateMap<LTC_COMMFILE, CommFile>();
