...
namespace KMHC.SLTC.Business.Implement
{
    public class DictManageService : BaseService, IDictManageService
    {
        ...
        public BaseResponse<IList<CodeValue>> QueryCode(BaseRequest<CodeFilter> request)
        {
            ...
            var q = from m in unitOfWork.GetRepository<NCI_CODEDTL>().dbSet
                    select m;
            List<NCI_CODEDTL> list = null;
        ...
        {
            return base.Get<NCI_CODEDTL, CodeValue>((q) => q.ITEMCODE == id && q.ITEMTYPE == pId);
        }

        public BaseResponse<CodeValue> SaveCode(CodeValue request)
        {
            return base.Save<NCI_CODEDTL, CodeValue>(request, (q) => q.ITEMCODE == request.ItemCode && q.ITEMTYPE == request.ItemType);
        }

        public int DeleteCode(string id, string pId)
        {
            Expression<Func<NCI_CODEDTL, bool>> filter = p => p.ITEMCODE == id && p.ITEMTYPE == pId;
            return base.Delete<NCI_CODEDTL>(filter);
        }
