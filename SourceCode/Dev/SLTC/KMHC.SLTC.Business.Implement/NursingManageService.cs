...
namespace KMHC.SLTC.Business.Implement
{
    public class NursingManageService : BaseService, INursingManageService
    {
        ...
        #region 護理需求
        public BaseResponse<IList<Evaluation>> QueryNurDemandEvalList(NursingFilter request)
        {
            BaseResponse<IList<Evaluation>> response = new BaseResponse<IList<Evaluation>>();
            ...
            {
                StringBuilder sb = new StringBuilder();
                string sql = string.Format(@"SELECT REG.REGNO,REG.NAME,REG.SEX,IPD.FEENO,Q.ID, Q.EVALDATE,Q.NEXTEVALDATE,
                                (SELECT EMPNAME FROM LTC_EMPFILE WHERE Q.EVALUATEBY=EMPNO) AS EVALUATEBY,
                                (SELECT EMPNAME FROM LTC_EMPFILE WHERE Q.NEXTEVALUATEBY=EMPNO) AS NEXTEVALUATEBY
            ...
            if (request.Data.KeyWord != "$$$")
            {
                response = base.Query<LTC_MEDICINE, Medicine>(request, (q) =>
                {
                    if (!string.IsNullOrEmpty(request.Data.KeyWord))
                    {
                        ...
                        q = q.Where(m => m.CHNNAME.Trim().Contains(request.Data.KeyWord) || m.ENGNAME.Trim().Contains(request.Data.KeyWord) || m.INSNO.Trim().Contains(request.Data.KeyWord) || m.HOSPNO.Trim().Contains(request.Data.KeyWord) || m.MEDCODE.Trim().Contains(request.Data.KeyWord));
                    }
