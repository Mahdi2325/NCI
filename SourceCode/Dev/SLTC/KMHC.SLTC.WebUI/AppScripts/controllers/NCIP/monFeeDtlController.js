...
.controller("rsMonFeeDtlCtrl", ['$scope', '$location', '$state', '$stateParams', 'rsFeeMonDtlRes', 'utility', function ($scope, $location, $state, $stateParams, rsFeeMonDtlRes, utility) {
    var NsMonFeeID = $state.params.id;
    var RsMonFeeID = $state.params.rsMonFeeId;
    var YearMon = $state.params.YearMon;
    var IdList = $state.params.idlist;
    ...
    $scope.closeRSMonFeeDtl = function () {
        $state.go('monFeeDetail', { id: $scope.NsMonFeeID, idlist: $scope.IdList });
        $state.stateName = "monFeeDetail";
    };
