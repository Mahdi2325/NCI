...
namespace KMHC.SLTC.WebController
{
    public partial class DC_ReportController : ReportBaseController
    {
        ...
        {
            //新進住民環境介紹記錄表  LTC_NEWEnvironmentRec
            //新進住民環境適應及輔導記錄  LTC_NEWEnvironmentTutor
        ...
        private void LtcNewResideEntenvRecOperation(WordDocument doc, ReportRequest request)
        {
            ...
            response.Data.Add
               (new
               {
                   ...
                   CONTRACTFLAG = ((bd.Data.CONTRACTFLAG == true) ? "01.契約內容說明" : "") + ((bd.Data.LIFEFLAG == true) ? "01.契約內容說明" : ""),

                   STAFF1 = bd.Data.STAFF1,
                   REGULARACTIVITY = ((bd.Data.REGULARACTIVITY == true) ? "01.定期活動說明" : "") + ((bd.Data.NOTREGULARACTIVITY == true) ? "02.不定期活動說明" : ""),

                   STAFF2 = bd.Data.STAFF2,
                   name = request.feeName,
                   BELLFLAG = ((bd.Data.BELLFLAG == true) ? "01.緊急鈴" : "") +
                              " " + ((bd.Data.LAMPFLAG == true) ? "02.床頭燈" : "") +
                              " " + ((bd.Data.TVFLAG == true) ? "03.電視" : "") +
                              " " + ((bd.Data.LIGHTSWITCH == true) ? "04.電燈開關" : "") +
                              " " + ((bd.Data.ESCAPEDEVICE == true) ? "05.逃生設備" : "") +
                              " " + ((bd.Data.ENVIRONMENT == true) ? "06.公共環境" : "") +
                              " " + ((bd.Data.COMMUNITYFACILITIES == true) ? "07.社區設施" : "") +
                              " " + ((bd.Data.POSTOFFICE == true) ? "郵局" : "") +
                              " " + ((bd.Data.SCHOOL == true) ? "學校" : "") +
                              " " + ((bd.Data.BANK == true) ? "銀行" : "") +
                              " " + ((bd.Data.STATION == true) ? "車站" : "") +
                              " " + ((bd.Data.PARK == true) ? "公園" : "") +
                              " " + ((bd.Data.TEMPLE == true) ? "寺廟" : "") +
                              " " + ((bd.Data.HOSPITAL == true) ? "醫療院所" : "") +
        ...
        private void ActivityReport(WordDocument doc, ReportRequest request)
        {
            ...
            if (bd.Talkedwilling != null)
            {
                ...
                {
                    bd.Talkedwilling = "有意願及選擇參與之活動內容";
                }
            }
            if (bd.Talkednowilling != null)
            {
                ...
                {
                    bd.Talkedwilling = "無意願參與活動將排資源介入或個別輔導";
                }
            }
            if (bd.Nottalked != null)
            {
                ...
                {
                    bd.Nottalked = "因生理/心理/認知等障礙由社工預計參與活動";
                }
                else
                {
                    bd.Nottalked = "";
        ...
        private void NutritionCareReport(WordDocument doc, ReportRequest request)
        {
            ...
            if (ncList != null && ncList.Count > 0)
            {
                ...
                for (var j = 0; j < colCount; j++)
                {
                    ...
                    dr[(j + 1).ToString()] = "正餐 " + dinnerFreqP1 + " 次/天\r點心 " + dinnerFreqP2 + " 次/天\r其他 " + dinnerFreqP2 + " 次/天";
                }
                dt.Rows.Add(dr);
                ...
                for (var j = 0; j < colCount; j++)
                {
                    dr[(j + 1).ToString()] = ncList[j].Kcal + "kcal/天\r" + "(主食類 " + ncList[j].KcalFood + " 份、\r肉魚豆蛋 " + ncList[j].KcalFish + " 份、蔬菜 " + ncList[j].KcalVegetables + " 份、\r水果類 " + ncList[j].KcalFruit + " 份、\r油脂類" + ncList[j].KcalGrease + " 份)";
                }
                dt.Rows.Add(dr);
                ...
                for (var j = 0; j < colCount; j++)
                {
                    dr[(j + 1).ToString()] = ncList[j].PipeKcal + "kcal/天\r" + "蛋白質 " + ncList[j].PipeProtein + " g、\r沖管水量 " + ncList[j].PipleWater + "c.c.、\r其他水量 " + ncList[j].PipleOtherWater + " c.c.、\rVit補充 " + ncList[j].PipleVit;
                }
                dt.Rows.Add(dr);
